...
.controller('ShortenController', function ($scope, $location, Links) {
  ...
  $scope.addLink = function(isValid) {
    console.log('addLink, isValid: ', isValid);
    if (isValid) {
      $scope.urlToShorten = /^http.*/.test($scope.urlToShorten) ? $scope.urlToShorten : 'http://' + $scope.urlToShorten;
      Links.addOne({ url: $scope.urlToShorten });
    }
  };
});
